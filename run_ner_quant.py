...
def main():
    ...
    parser.add_argument('--prune_mask', type=str, default=None, help="Weight mask for pruning weights.")
    parser.add_argument('--quantize', action='store_true',
                        help="Whether the model should be trained using QAT, then quantized to 8 bit.\n"
                             "If in eval mode, a previously quantized model will be loaded instead.")
    ...
    ## Pruning 
    if args.prune_mask is not None:
        ## Load mask dict
        prune_mask = torch.load(args.prune_mask)
        ## Apply masks directly to weights and permanently prune them
        for key in prune_mask.keys():
            attrs = key.split(".")
            ...
            tensor_mask = prune_mask[key].to(torch.device("cpu"))
    ...
    if args.quantize:
        ...
        model.bert.embeddings.word_embeddings.qconfig = emb_qconf
        model.bert.embeddings.position_embeddings.qconfig = emb_qconf
        model.bert.embeddings.token_type_embeddings.qconfig = emb_qconf
    ...
    if args.do_train:
            ...
            if args.do_eval and (args.local_rank == -1 or torch.distributed.get_rank() == 0):
                ...
                for input_ids, input_mask, segment_ids, label_ids,valid_ids,l_mask in tqdm(eval_dataloader, desc="Evaluating"):
                    ...
                    for i, label in enumerate(label_ids):
                        ...
                        # print("--------------------")
                        # print("label:")
                        # print(*[m for j, m in enumerate(label) if j > 0])
                        # print("label ids:")
                        # print(*[label_ids[i][j] for j, _ in enumerate(label) if j > 0])
                        # print("logits:")
                        # print(*[logits[i][j] for j, _ in enumerate(label) if j > 0])
                        for j,m in enumerate(label):
                            if j == 0:
                                ...
                                logit = logits[i][j]
                                # TODO: this might not be the ideal fix
                                if logit == 0:
                                    logit = len(label_map)
                                temp_2.append(label_map[logit])
                ...
                with open(output_eval_file, "a+") as writer:
                    ...
                    writer.write(report)
                    writer.write("\n")
                    writer.write("Accuracy: " + str(accuracy_score(y_true, y_pred)))
                    writer.write("\n")
        ...
        ## Quantization
        # finish quantizing model and save 8-bit version
        model.to(torch.device("cpu"))
        qat.on_fit_end(None, model)
        torch.save(model.state_dict(), os.path.join(args.output_dir, '8bit_model.pt'))
        ...
        if args.quantize:
            qat.on_fit_start(None, model)
            qat.on_fit_end(None, model)
            model.load_state_dict(torch.load(os.path.join(args.output_dir, '8bit_model.pt')))
